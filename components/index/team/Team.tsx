...
const Team = ({ teamItems }: Props) => {
  const [currentTeamItem, setCurrentTeamItem] = useState<TeamItem>(
    teamItems[0]
  );
        ...
        <div className="max-w-[700px] justify-around h-full flex-1 flex flex-col m-auto px-3 pl-5 text-center lg:pr-[5vw]">
          ...
          <div data-aos="fade-left" data-aos-duration="1000">
            <h2 className="text-[3rem] font-semibold">
              {currentTeamItem.name}
            </h2>
            <p
              className="mt-2 mb-10 text- md:text-2xl"
              dangerouslySetInnerHTML={{ __html: currentTeamItem.description }}
            ></p>
          </div>
          <div className="flex items-end mb-10  lg:mb-0">
            {teamItems &&
              teamItems.map((e, index) => {
